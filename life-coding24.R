...
for (i in 1:sim) {
  data[, i] <- time.series(10, tmax, possible_outcomes, probabilities)
}
...
data.mean <- time.series.lambdamean(10, tmax, lambda_mean)
data.geo <- time.series.geo(10, tmax, lambda_mean)

matplot(log(data), type = "l", lty = 1, xlab = "time", ylab = "population size")
matlines(log.row.means, type = "l", lty = 1, col = "red")
matlines(log(data.mean), type = "l", lty = 1, lwd = 2, col = "blue")
matlines(log(data.geo), type = "l", lty = 1, lwd = 2, col = "black")
  
-------------
matplot(log(data+1), type = "l", lty = 1, xlab = "time", ylab = "population size")
matlines(log(rowMeans(data)+1), type = "l", lty = 1, col = "red")
matlines(log(data.mean+1), type = "l", lty = 1, lwd = 2, col = "blue")
matlines(log(data.geo+1), log = "y", type = "l", lty = 1, lwd = 2, col = "black")

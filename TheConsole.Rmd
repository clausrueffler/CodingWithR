...
4) Editor panel for *R*-scripts. This window appears in the top left once you opened an *R*-script.
...
The logical not-operator can be useful in data filtering.

```{r eval=FALSE}
! TRUE #  negates a logical value or expression, turning TRUE into FALSE and vice versa
! FALSE
! (5 > 3)

x <- c(2, 4, 6, 8, 4)
! (x == 4) # checks where entries of the vector x are NOT equal to 4
x[! (x == 4)] # returns the vector without the entry 4
```
...
A %*% D # matrix operations are not defined for matrices with character entries
```

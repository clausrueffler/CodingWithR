...
Use descriptive file names for your scripts; use underscore (_) or dots (.) to separate words.
...
Good function names (use verbs if it makes sense)
...
As a slightly less trivial example, let us calculate the factorias of *n*, $n! = 1 * 2 * ... * (n-1) * n$ using a ```for```-loop.
...
The ```while``` loop is used when the commands need to be repeated while a certain condition is true.
...
This script performs exactly the same operations we wrote for the ```for``` loop above. Note that you need to update the value of ```i``` (using ```i <- i + 1```), otherwise the loop will run forever (infinite loop — to terminate click on the "Stop" button in the top-right corner of RStudio). 
...
circle_surface(1:5) # applies the function to the numbers 1 to 5 in a single step; thus, we apply functions to vectors
```

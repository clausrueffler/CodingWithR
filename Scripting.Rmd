...
```{r eval=FALSE}
data <- time.series(tmax = 20, lambda = 1.1, N0 = 10) # we assign the output of the function to n. It is only now that n appears in your environment tab and we can now use it outside the function.
plot(data)
...
However, we add a command to a function so that it automatically *exports* the output to the global environment using the ```<<-``` arrow:

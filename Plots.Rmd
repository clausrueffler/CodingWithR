...
The plotting command ```matplot(x, y, ...)``` in R is an easy way to superimpose several data series. Here, *x* is a vector containing the values for the x-axes and *y* is a matrix containing the data as a function of *x*. Note, that ```matplot(x, y, ...)``` plots each *column* of *y* as separate data series.

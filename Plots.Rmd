...
plot(x, y, col = "orange", xlab = "my x-label", ylab = "a y-label") # add axes labels
plot(x, y, xlim = c(1,35), ylim = c(1,40)) # set plot ranges
```

### Function Plots

We can plot the graphs of functions using the command ```curve(expr, from, to, ...)```, where *expr* is a mathematical expression containing an argument *x* or function name, and *from* and *to* defines the range of *x-values for which the function is plotted. This is a useful tool for visually exploring the shape of a function.
...
matplot(x, cbind(y.1, y.2), col =c(2, 4), pch = c(0, 2), xlab = "x", ylab = "y.1 = x^2 and y.2 = x^2-2*x")

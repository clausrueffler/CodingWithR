...
```
Within this basic plotting command, we can add many additional arguments to change to the exact layout of the plot. For a more thorough overview of the options see [here](https://r-charts.com/base-r/line-types/).

```{r eval=FALSE}
plot(y, type = "l") # using lines instead of points
plot(y, type = "l", lty = 2) # change the line-type, the value 2 produces a dashed line.
...
plot(y, log="y") # plots the same data but with the y-axis on a log-scale (the same can be done with the x-axis)
```
...
abline(a = 0, b = 1, col = "red") # add a line to the same plot by specifying its intercept a and slope b
```
